/**
 * Map internal pipeline errors to short user-facing messages. Stops
 * the response from leaking SSRF internals (resolved IPs, path
 * hints) or stack details that would signal "that attack got through
 * to the crawler".
 */
export function scrubError(raw: string): string {
  ...
  if (lower.includes("http 403") || lower.includes("bot challenge"))
    return "This site blocked the crawler."
