...
These have wildly different resource costs, so "how many requests per second" has three answers:
...
3. **Supabase Pro connection-pool ceiling** is far above actual usage because the service-role client is cached one-per-Fluid-instance.
...
2. **Connection pool pressure.** Supabase's PgBouncer handles many concurrent connections out of the box; because the service-role client is cached one-per-Fluid-instance, the open-connection count scales with instance count, not request count. Only becomes a bottleneck at very high concurrent-instance counts — switch to Supabase's transaction-mode pooler when that happens.
...
### What still caps the system at enterprise tier

Even with unlimited budget:

1. **Anthropic TPM** remains the core ceiling for LLM-enriched output. Each crawl's ~20 K tokens is the lever to cut for more throughput per dollar — smaller prompts, fewer enrich batches, or cheaper-model-per-step routing.
2. **Puppeteer memory for SPA-heavy workloads** — only if the Browserless offload isn't in place.
3. **Target-site politeness** is an inherent ceiling; crawling any single domain faster than ~5 req/s is rude regardless of available capacity.
