...
| **Anthropic API rate limits** | **Shipped — three layers.** (1) `getClient()` in `lib/crawler/enrich/llmEnrich.ts` passes `maxRetries: llm.MAX_RETRIES` (5) and `timeout: llm.CALL_TIMEOUT_MS` (60 s) to the SDK, which retries 408 / 429 / 5xx with exponential backoff and honours `retry-after` / `x-should-retry` response headers. (2) Per-crawl call count is held to 4 by bundling related work into shared round trips: `rankSiteUrls` ranks both internal URLs and external references in one call; `analyzeSiteHomepage` picks the brand name and writes the preamble in one call. (3) The monitor cron passes a `delaySeconds` stagger to `enqueueCrawl` (`monitor.RECRAWL_STAGGER_SEC = 5`, capped at `RECRAWL_STAGGER_MAX_SEC = 300`), so N drifted URLs fan out over time instead of firing N×4 LLM calls in one minute. User submissions still enqueue with delay 0 — only cron callers stagger. | A bursty minute waits through the backoff window. If retries exhaust (or any LLM call site fails for any reason — missing key, billing exhausted, transport error), the call site throws `LlmUnavailableError` and the job lands as `failed` with a user-visible "AI service is unavailable" message — there is no degraded-quality success path. | Open follow-up: switch to QStash Queues (with `parallelism`) once available on plan, for an explicit concurrency ceiling on top of the producer-side stagger. Past that, the Anthropic tier upgrade is the next lever — Tier 2 (1000 RPM) makes the stagger optional at realistic batch sizes. |
...
| **Monitor cron throughput** | 200 pages per invocation, sequential with 400 ms same-host delays, daily (Vercel Hobby-plan ceiling). Also force-fails stuck jobs past `STUCK_JOB_AFTER_MS` = 15 min at the head of each run. Drift-triggered re-crawls are enqueued with a per-job stagger (`monitor.RECRAWL_STAGGER_SEC = 5`, capped at `RECRAWL_STAGGER_MAX_SEC = 300`) so a tick where many URLs drifted doesn't fan out N workers in the same minute and burn the Anthropic RPM budget. | At ~1 M monitored URLs the cron can't check them all in a day, let alone in one 300 s invocation. | Tighten the schedule to hourly once on Pro plan, move to a sharded cron (multiple invocations per day, each handling a URL-hash range), or enqueue signature checks onto the queue (above) so they fan out across workers. |
