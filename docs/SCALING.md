...
| **Anthropic API rate limits** | **Shipped.** `getClient()` in `lib/crawler/enrich/llmEnrich.ts` passes `maxRetries: llm.MAX_RETRIES` (5) and `timeout: llm.CALL_TIMEOUT_MS` (60 s) to the SDK, which already retries 408 / 429 / 5xx with exponential backoff and honours `retry-after` / `x-should-retry` response headers. | A bursty minute waits through the backoff window. The 5-retry budget covers a typical Anthropic 429 burst within the pipeline budget. If retries exhaust (or any LLM call site fails for any reason — missing key, billing exhausted, transport error), the call site throws `LlmUnavailableError` and the job lands as `failed` with a user-visible "AI service is unavailable" message — there is no degraded-quality success path. | Open follow-up: when the Upstash integration supports concurrency controls (QStash Queues w/ `parallelism`), add a producer-side cap as defence-in-depth. For now the Anthropic tier is the sole concurrency ceiling — plan to upgrade tiers before traffic hits it. |
