...
| **Anthropic API rate limits** | SDK default retries only (`maxRetries: 2`, not overridden). Each crawl fires 3–5 calls (`rankCandidateUrls`, `enrichBatch`, `rankExternalReferences`, `llmSiteName`, `generateSitePreamble`); each step has its own deterministic fallback if retries exhaust. | At high concurrency we'll trip Haiku's per-account requests-per-minute limit. A failed call silently falls back to deterministic defaults, which is correct behaviour but quietly degrades output quality. | Wrap `client.messages.create` in a retry helper with a higher `maxRetries` + explicit exponential backoff. Add a global LLM semaphore (e.g. max 10 concurrent calls per instance) to smooth bursts into the provider. |
...
| **`/api/p/[id]` response caching** | **Shipped.** Terminal statuses (`complete`/`partial`/`failed`) respond with `Cache-Control: public, s-maxage=3600, stale-while-revalidate=86400`; in-flight polls stay `no-store` so progress remains live. `updateJob` calls `revalidatePath('/api/p/:id')` for every sibling job id when a terminal result is written, so monitor re-crawls invalidate historical job ids immediately (all map to the same `pages.result`). | In-flight polls still hit Postgres every 1.5 s — one read per concurrent active job. Multiplier is concurrent-user count, not total-URL count, so far below Postgres's ceiling at any realistic volume. | Open follow-up: short-TTL (~1 s) Redis cache on in-flight status if concurrent-job count ever becomes a DB-load concern. Not relevant at current volume. |
...
4. ~~**Edge-cache terminal `/p/[id]` responses**~~ — *shipped.* Terminal responses carry `Cache-Control: public, s-maxage=3600, stale-while-revalidate=86400`; `updateJob` revalidates every sibling job id's path on a terminal-result write so monitor re-crawls propagate to the CDN without waiting for TTL.
