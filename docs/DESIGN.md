...
Section ordering in the assembled file (`assemble.ts` → `groupBySection`) is **average-score first**, with a small `SECTION_PRIORITY` tiebreaker. The structural-vs-catalogue judgment ("About is foundational; an individual item in a directory of similar items is catalogue") lives in the LLM's per-page `importance` score — the `enrichBatch` prompt instructs the model to score structural pages (About, Company, Pricing, Products / Services, Docs / API, Support, Careers) at 8–10 and individual catalogue entries (one article in a feed, one product in a catalog, one listing in a marketplace, one bio in a team page) at 4–7. After that weighting flows through `scorePages`, structural sections naturally have higher avg-scores and rise to the top of the file. The deterministic `SECTION_PRIORITY` table only lists labels with stable cross-genre meaning in the llms.txt shape (About, Pricing, Docs, Support, etc.) and only acts as a tiebreaker — the assembler doesn't hardcode catalogue-shaped section nouns or genre-specific terms, so an LLM-invented section name gets the same neutral treatment regardless of what the underlying site happens to be. Optional is always last.
...
Six call sites, all pinned to the model in `llm.MODEL`:
...
6. **`llmFinalReview`** — runs after `assembleFile` produces a draft markdown. The model is given the **actual full file** (H1, summary, preamble, every section header, every entry's title / URL / description) and returns `{ drop_urls, section_order }`. Reading the whole assembled file at once lets it catch noise per-page enrichment can't see — login redirects, marketing-tracking-param URLs, individual catalogue items the section index already covers, descriptions that repeat the preamble — and lets it reorder sections when the avg-score sort put a foundational one (About, Pricing, Docs) below a catalogue. Drops are looked up via a rendered-URL → canonical-URL alias map so the model can return URLs in whatever form they appear inside the `[..](URL)` of the draft. No-op on parse error / empty response / unavailable LLM, leaving the draft untouched.
...
- **Integration / end-to-end test coverage.** A `node:test` suite under `tests/` covers the security-critical pure helpers (SSRF IP-range classifiers, URL normalisation, scrubError, validateLlmsTxt, robots / sitemap / spa-detection helpers, rate-limit math) and runs on every PR via `npm test`. The integrated paths — full crawl pipeline, RLS enforcement against a real Supabase, monitor cron, QStash delivery, browser flows — aren't in the automated suite; they're covered by the manual playbook in [`TESTING.md`](./TESTING.md). Adding integration coverage is the next test-investment step if the project moves beyond a take-home demo.
