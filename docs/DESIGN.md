...
1. **`rankCandidateUrls`** — reorders the candidate URL list by likely value. Takes the whole list + site name + homepage excerpt; skipped entirely for small lists where ranking doesn't buy anything. Caps in `lib/config.ts`. The LLM is required for this step — if the call fails (no API key, billing exhausted, transport error after SDK retries), the pipeline raises `LlmUnavailableError`, which the outer catch in `runPipelineInner` converts to a `failed` job with a user-facing "AI service is unavailable" message. The same applies to every other call site in this list. Producing a deterministic-only file when the AI was unavailable would mislabel a degraded result as a real one — better to surface the failure so the user can retry.
...
4. **`llmSiteName`** — picks a clean brand name from deterministic homepage candidates. Falls back to the deterministic guess only when the LLM responds with something that fails sanitization. Transport failures throw `LlmUnavailableError` like every other call site (job lands as `failed`).
