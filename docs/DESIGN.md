...
Before any of this, the URL discovery stage runs three deterministic filters in order: `dropParametricFanout`, `capByPathPrefix`, and `capTotalByPathDepth` (all in `lib/crawler/net/url.ts`).

**`dropParametricFanout`**: a first-path-segment subtree triggers as fan-out only when **both** signals fire: total URLs under it ≥ `crawler.FANOUT_DROP_THRESHOLD` (20) AND distinct second-segment values ≥ `max(8, threshold/2)`. Both signals together let `/city/{slug}` × hundreds and `/cities/{city}/venues/{slug}` × thousands trip (one distinct second-segment per city — high cardinality) while a real docs hierarchy like `/docs/{cat}/{topic}` × hundreds passes through (3–5 distinct depth-2 categories — low cardinality). When a first segment trips, every URL under it (any depth) is dropped, since deeper children of a fan-out section are also fan-out. The depth-1 parent index page (`/city`, `/store`, `/location`) lives in a different first-segment bucket and survives — that's what an LLM consumer of llms.txt actually needs from a directory-of-similar-items section. The LLM ranker's prompt also instructs it to drop fan-out, but the LLM is too unreliable on this one (it scores each fan-out page individually, where each looks legitimate), so the deterministic filter runs first.

**`capByPathPrefix`** then caps URLs per `(seg0, seg1)` bucket at `crawler.URLS_PER_PREFIX_CAP` (5), preferring shorter paths within each bucket so the section index wins the slot over individual leaves.

**`capTotalByPathDepth`** is the absolute pre-LLM ceiling: `llm.RANK_INPUT_MAX` (200) URLs total, sorted by path depth ascending so structural section indexes survive over deep leaves, with original input order as the tiebreaker. The bucket-based caps above bound URLs per prefix, but a sprawling enterprise site with many distinct top-level sections can still push hundreds of URLs through. The total cap puts a fixed lid on the LLM's input regardless of how bucket diversity shakes out, bounding worst-case Anthropic input-TPM consumption (200 URLs ≈ 12K input tokens — under 25% of Tier 1's 50K TPM per call).

Section ordering in the assembled file (`assemble.ts` → `groupBySection`) sorts by an **effective score** — `avgPageScore + (sectionPriority − 50) × 0.3`. The LLM's per-page `importance` carries the bulk of the judgment (the `enrichBatch` prompt scores structural pages at 8–10 and catalogue items at 4–7), but in practice the model bunches its scores in the middle, so the small additive boost from `SECTION_PRIORITY` lets a foundational-named section (About → +15, Services → +12, Support → +3) edge out a slightly-higher-scored catalogue section. The boost is calibrated so it flips small avg-score gaps but doesn't override a genuinely-stronger catalogue. The `SECTION_PRIORITY` table only lists labels with stable cross-genre meaning in the llms.txt shape (About, Pricing, Docs, Support, etc.) and never includes catalogue-shaped section nouns or genre-specific terms, so an LLM-invented section name gets the neutral default (50). After this sort, the LLM final-review pass (§7 #4) can override the order entirely with an explicit `section_order`. Optional is always last.

Then on top of that — once the file is assembled — the LLM final-review pass (`llmFinalReview`, §7 #4) can both reshape the section set (by `moves` that reassign entries between sections, including merging singletons into a topically-adjacent larger section, with new section names allowed) and override the ordering entirely by returning an explicit `section_order`, plus override the in-section order by returning a per-section `entry_order` URL list. Moves are applied before assembly re-runs, `section_order` validates against the post-move section set, and `entry_order` reorders entries within a section so adjacency carries meaning (chronological release notes grouped newest-first, narrative reading order on docs, sibling product variants kept adjacent, indices above the leaves they index). That's what catches cases where the avg-score sort still got it wrong despite the boost, e.g. on a site whose catalogue is so dominant that no boost wins, where per-page enrichment fragmented related entries across multiple thin headers, or where related items in the same section landed in non-meaningful order.
...
Same-domain filtering on the crawl queue is correct (a single outbound anchor would otherwise give the crawler permission to roam the web), but leaves the output with no cross-origin entries even when the site explicitly links to its own spec, upstream docs, or closely related projects. External references bridge that gap: homepage outbound anchors only, LLM-ranked down to `crawler.EXTERNAL_REFS_MAX_KEEP` inside the unified `rankSiteUrls` call (alongside internal-URL ranking, in one round trip), then `fetchPreRankedExternals` fetches each exactly once for metadata (no link discovery, no further LLM calls). The fetched entries are merged into scoring alongside internal pages so the LLM can place them in whatever section fits. SSRF is preserved — `fetchPage` still goes through `safeFetch` with per-hop validation.

---

## 7. LLM Usage

Four call sites per crawl, all pinned to the model in `llm.MODEL`. Two of them bundle related work into a single round trip — `rankSiteUrls` ranks both internal URLs and external references in one call, `analyzeSiteHomepage` picks the brand name and writes the preamble in one call — keeping the per-crawl count low matters for staying inside the per-tier RPM budget when monitor-cron fan-out spikes.
...
2. **`enrichBatch`** (via `llmEnrichPages`) — returns `importance` (1–10), `section` label, `description`, and an optional `label` per page. `ENRICH_BATCH_SIZE` is sized to match `MAX_PAGES`, so a typical full-budget crawl ships in one batch / one round trip; the per-batch parallel-fan-out path remains in `llmEnrichPages` as a safety mechanism if either cap is raised in the future. `section` is the primary signal for pages above the primary threshold; the path-regex inference only runs when the LLM didn't return a usable section. Pages below the primary threshold always land in `Optional` regardless of what the LLM said. The prompt enforces a hard size band of **6–9 entries per section** as a quality floor on grouping — crossing 9 entries is a per-page-enrichment failure signal that tells the model to back up and split the section by an orthogonal axis (audience / surface / purpose / domain / lifecycle). On B2B / SaaS / platform sites specifically, the prompt requires aggressive splitting of business pages from product pages: paths under `/business/`, `/enterprise/`, `/merchants/`, vertical-solution paths (`/business/ai-for-finance/`), or titles containing "Business" / "Enterprise" / "for Teams" / "for Engineering" land in a **Business** (or **Enterprise** / **Solutions**) section, never in **Products**, since the audience and intent differ even when the underlying product is the same. `label` is the optional first-pass link-text override, biased hard toward leaving the raw `<title>` alone — only set on objectively broken cases (run-together URL slugs like "Getstarted", site-name-as-title on a non-homepage page, marketing boilerplate too long to read as link text). The deterministic resolver in `assemble.ts` runs on top of whatever the LLM returns: site-name collapse, generic-title fallback, and collision-disambiguation (path-prefix differentiation when two entries land on the same label) all still apply, with `displayLabel || title` as the input. The cross-batch / context-aware override layer for labels lives in `llmFinalReview` (job #4) and wins on conflict.
...
4. **`llmFinalReview`** — runs after `assembleFile` produces a draft markdown. The model is given the **actual full file** (H1, summary, preamble, every section header, every entry's title / URL / description) and returns `{ drop_urls, moves, section_order, relabel, redescribe, entry_order }`. Reading the whole assembled file at once lets it catch noise per-page enrichment can't see — login redirects, marketing-tracking-param URLs, individual catalogue items the section index already covers, descriptions that repeat the preamble — reassign entries that landed under the wrong header (a "Player Inclusion Coalition" entry placed under About should be in Community), consolidate fragmented sections by moving singleton entries into a topically-adjacent larger section (target name may be an existing section or a new merged label), **split oversized sections** when any single section holds more than 9 entries (target band 6–9; the prompt prioritises this over fixing 1–2 outliers and gives the same orthogonal-split-axis guidance — audience / surface / purpose / domain / lifecycle — that `enrichBatch` works from, so this layer can fix what the per-page pass missed), reorder sections when the avg-score sort put a foundational one (About, Pricing, Docs) below a catalogue, rewrite link labels that came out awkwardly (run-together URL slugs like "Getstarted" → "Get Started", URL-derived fallbacks like "Page1") — overriding any first-pass `label` from `enrichBatch` — redescribe entries whose description reads strangely next to its label (grammatical errors, vague marketing prose, structure references, label-repeating openings, off-language), and reorder entries within a section to group related items together (chronological release notes newest-first, narrative reading order on docs, sibling product variants kept adjacent, indices above the leaves they index). The `entry_order` arm accepts any section as a target including `Optional` — release notes scattered across `Optional` in non-chronological order is the canonical case it fixes. Moves + description rewrites are applied first so any new section names they introduce are eligible to appear in `section_order` and so the re-rendered draft contains the corrected text. Drops, moves, relabels, redescribes, and entry-order URL lists are matched via a rendered-URL → canonical-URL alias map so the model can return URLs in whatever form they appear inside the `[..](URL)` of the draft. A description rewrite bumps `descriptionProvenance` to `"llm"` so `assemble.ts` keeps rendering the description (it gates render on provenance ≠ `"none"`). A parse error or "no edits" response leaves the draft untouched; transport failures throw `LlmUnavailableError` and surface the job as `failed`. Code-level safety net in `pipeline.ts`: an excessive `drop_urls` response (would drop > 20% of entries OR leave the file with fewer than 5 entries) is rejected wholesale and the draft ships unedited — the prompt also tells the model not to do this, but the code refuses regardless of what the model returned. Logged as `[llmFinalReview] rejected destructive drop: N/T entries (would leave M)` for ops visibility.
