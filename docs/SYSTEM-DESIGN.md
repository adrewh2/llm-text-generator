...
Each link line is `- [label](url): description`. The label resolution chain has three layers in priority order: (1) `enrichBatch` may emit an optional first-pass `label` per page when the raw `<title>` is objectively broken (run-together URL slug, site-name-as-title, marketing boilerplate); (2) on top of that, the deterministic resolver runs site-name collapse, generic-title fallback (when the same label repeats across many pages — a common SPA failure where `document.title` never updates), and collision disambiguation by prefixing the first differing path segment; (3) `llmFinalReview` `relabel` overrides win on top of both — that's the cross-batch / context-aware override layer.

Once the draft is assembled, one more LLM call (`llmFinalReview`) hands the **whole rendered file** to the model and asks six questions: are any entries clear noise that should be dropped given the rest of the file, do any entries belong under a different section (a misclassified item, a singleton section that should be consolidated into a topically-adjacent larger one, or — most importantly on B2B / SaaS / platform sites — an oversized section holding more than 9 entries that needs to split by an orthogonal axis like audience / surface / purpose), is the section ordering right for an llms.txt (foundational sections first, catalogue / news / blog later), are there any link labels that read awkwardly (run-together URL slugs like "Getstarted", URL-derived fallbacks) — overriding any first-pass `label` from `enrichBatch` — are there any descriptions that read strangely next to their label (grammatical errors, vague marketing prose, structure references like "the homepage of…", off-language), and within any section are there related entries that landed in scattered positions (chronological release notes that should group newest-first, sibling product variants split across the section by score, narrative reading order on docs)? The 6–9 entry-per-section band is enforced at both layers: `enrichBatch` is told to fragment up front, and final review is told to split anything that crossed the ceiling — the two-layer reinforcement matters when per-page enrichment under-fragments because it can't see the global shape of the file. Reading the assembled file at once lets the model catch what per-page enrichment can't see — login-redirect URLs, individual catalogue items the section index already covers, descriptions that just repeat the preamble, sentence-pair quality issues that only appear once label and description sit next to each other, in-section adjacency that only matters in the rendered view. The model returns `{ drop_urls, moves, section_order, relabel, redescribe, entry_order }`; the file is re-assembled with the moves + description rewrites applied first (so new section names introduced by consolidation participate in `section_order`), the drops applied, the LLM-supplied section order honored over the avg-score sort, per-URL label rewrites overriding the deterministic resolver, and per-section `entry_order` URL lists overriding the default in-section sort (the `Optional` section is a valid target — release notes scattered there is the canonical case it fixes). A parse error or empty edit response is a silent no-op so the draft survives unchanged; transport failures (no API key, billing exhausted, exhausted SDK retries on 5xx / 429) throw `LlmUnavailableError` and surface the job as `failed`.
