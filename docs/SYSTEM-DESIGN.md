...
Before committing to a final list, the queue is trimmed. A per-section cap stops a deep documentation subtree from swamping the crawl. The site's primary language is detected and obvious off-locale prefixes like `/fr/`, `/de/`, `/ja/` are dropped. Then the queue goes to the LLM for ranking — pick the URLs a reader of `llms.txt` would actually want.
