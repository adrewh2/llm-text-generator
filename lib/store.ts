 ...
 * is the per-execution identifier (powers /jobs/{jobId}, the live
 * progress view) and is generated by the caller; `pageId` is the
 ...
 * first-paint and by the /api/p/[id] route handler when the page's
 * latest job is terminal. For in-flight reads, prefer
 * `getPageStatusById` — it skips the heavy result + crawled_pages
 * columns.
 */
